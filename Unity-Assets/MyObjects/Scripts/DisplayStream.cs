...
// reçoit le flux de la vidéo à partir d'une URL de type: http://192.168.2.216:5000/video
// et qui l'applique à une texture pour l'afficher dans Unity
...
public class DisplayStream
{
    ...
    // il se peut que le serveur pour la vidéo mette du temps à se lancer et ne soit pas disponible tout de suite
    // il faut donc réessayer plusieurs fois de s'y connecter
    ...
    private int halfImageW = 0; // moitié de la largeur de l'image
...
/*                            if (frame.Length < imageLength)
                            {
                                // Si la taille du tableau frame est inférieure à la taille de l'image actuelle, réallouez le tableau.
    ...
    public void StopStream()
    {
        streamRunning = false;
        streamStopped = true;
        Debug.Log("arrêt de l'affichage du stream");
    }
    ...
    /*    private void DisplayImage(byte[] imageData)
        {
            ...
            if (currentTexture.LoadImage(imageData))
            {
                if (useMaterial)
                {
                    if (oldTextureH != currentTexture.height || oldTextureW != currentTexture.width) // si la taille de l'image a changé
                    {
    ...
    private void DisplayImage(byte[] imageData)
    {
        ...
        if (currentTexture.LoadImage(imageData))
        {
            if (useMaterial) // s'il sagit d'une image stéréo
            {
                if (ImageH != currentTexture.height || halfImageW != currentTexture.width/2) // si la taille de l'image a changé
                {
                    ImageH = currentTexture.height;
                    halfImageW = currentTexture.width/2;

                    // l'angle des caméras est de 60° (on multiplie par 3 pour afficher sur 180°)
                ...
                // Copier les pixels des images stéréo aux bons endroits dans la texture finale de manière à créer un cadre noir autour de chacune
    ...
    // Fonction pour obtenir la moitié de l'image stéréoscopique
    ...
    // en fonction de la texture de l'image passé en paramètre
    private Texture2D CreateTexture(Texture2D texture)
    {
        // valable que si l'angle de la caméra est de 60°

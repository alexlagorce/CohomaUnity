...
{
    public string streamURL; // URL de votre flux vid�o, par exemple "http://192.168.2.216:5000/video"
                ...
                if (imageLength > 0)
                {
                    ...
                    if (init == 0)
                    {
                        ...
                        if (videoTexture.LoadImage(frame))
                        {
                            ...
                            // on ajoute des pixels noir autour de la vid�o pour pouvoir afficher la vid�o sur 120� (car le rendu est affich� obligatoirement sur 180�)
                            ...
                            // Calculer la position de d�part pour copier l'image originale
    ...
    /*    private void DisplayImage(byte[] imageData, Texture2D newTexture, int startX, int startY)
        {
            ...
            if (videoTexture.LoadImage(imageData))
            {
                ...
                newTexture.Apply(); // Appliquer les changements � la texture
                displayMaterial.mainTexture = newTexture; // Utiliser la nouvelle texture dans le mat�riau
            }
            else
            {
    ...
    private void DisplayImage(byte[] imageData)
    {
        ...
        // Chargement des donn�es d'image dans la texture
